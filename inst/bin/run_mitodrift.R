...
option_list <- list(
    ...
    make_option(
        ...
        type = "logical",
        default = TRUE,
        help = "Whether to fit parameters using MCMC",
        metavar = "LOGICAL"
    ),
    make_option(
        c("-t", "--fit_param_steps"),
        type = "integer",
        default = 500,
        help = "Number of MCMC steps for parameter fitting",
        metavar = "INTEGER"
    ),
    make_option(
        c("-c", "--fit_param_chains"),
        type = "integer",
        default = 1,
        help = "Number of MCMC chains for parameter fitting",
        metavar = "INTEGER"
    ),
    make_option(
        c("-d", "--fit_param_keep"),
...
param_trace_file <- file.path(opts$outdir, "fit_param_trace.csv")
...
if (opts$resume) {
    ...
    if (opts$fit_params) {
        message("\n=== Fitting parameters using MCMC ===")
        fitted_params <- md$fit_params(
            nsteps = opts$fit_param_steps,
