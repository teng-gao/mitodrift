...
//     std::vector< std::vector<double> > logP_list, std::vector<double> logA,
//     std::vector<double> log_messages) {
//     // Make copies for the two alternative topologies.
//     arma::Mat<int> E1 = E;
//     arma::Mat<int> E2 = E;
...
//     // Find the nth internal edge (using the original loop).
//     int count = 0, ind = -1;
//     for (int i = 0; i < numEdges; i++) {
//         if (p_child[i] > nTips) {
//             count++;
...
//     // Retrieve children values for the swapping.
//     int e1_val = p_child[ind1];
//     int e2_val = p_child[ind2_0];
//     int e3_val = p_child[ind2_1];
...
//     // --- Now, determine the subtrees that are affected by the swap.
//     // For each of e1, e2, and e3 (which are node labels), get the edge indices in their subtrees.
//     // Note: if a node is a tip, its subtree is empty.
    
//     // First, build the auxiliary arrays that you already computed in reorderRcpp.
//     int m = parent.max(); 
//     int nnode = m - nTips;
//     std::vector<int> L(numEdges);            // Will store edge indices per internal node.
//     std::vector<int> pos(nnode, 0);            // Current fill position for each node.
//     std::vector<int> xi(nnode, 0);             // Starting index for each node in L.
//     std::vector<int> xj(nnode, 0);             // Count of children per internal node.
...
//     for (int i = 0; i < numEdges; i++) {
//         int idx = p_parent[i] - nTips - 1;
//         xj[idx]++;
//     }
//     // Compute cumulative starting positions.
//     for (int i = 1; i < nnode; i++) {
//         xi[i] = xi[i - 1] + xj[i - 1];
//     }
//     // Fill L.
//     for (int i = 0; i < numEdges; i++) {
//         int k = p_parent[i] - nTips - 1;
//         int j = pos[k];
//         L[xi[k] + j] = i;
//         pos[k]++;
//     }
    
//     // Now, for each node e1_val, e2_val, and e3_val, get their subtree edge indices.
//     std::unordered_set<int> subtree_e1 = getSubtreeEdges(e1_val, nTips, L, xi, xj, child);
//     std::unordered_set<int> subtree_e2 = getSubtreeEdges(e2_val, nTips, L, xi, xj, child);
//     std::unordered_set<int> subtree_e3 = getSubtreeEdges(e3_val, nTips, L, xi, xj, child);
    
//     // Union the three sets.
//     std::unordered_set<int> unionSubtrees = subtree_e1;
//     unionSubtrees.insert(subtree_e2.begin(), subtree_e2.end());
//     unionSubtrees.insert(subtree_e3.begin(), subtree_e3.end());
    
//     // Finally, reorder the trees (as before) for output.
//     std::vector<arma::Mat<int>> Ep(2);
//     Ep[0] = reorderRcpp(E1);
//     Ep[1] = reorderRcpp(E2);

//     score_tree_bp_wrapper2(Ep[0], logP_list, logA, unionSubtrees, )
...
// // bp function: Belief-propagation function.
// // logP is a flattened likelihood matrix (row-major; dimensions: C x n)
// // logA is a flattened transition matrix (dimensions: C x C)
// // E is a flattened edge list in postorder (each edge: parent, child)
// // n: number of nodes, C: number of states, m: number of edges, root: root node index.
// // nodes_skip (optional) is a vector of nodes that will be skipped in the log_messages computation.
// // [[Rcpp::export]]
// double score_tree_bp2(std::vector<int> E,
//                       std::vector<double> logP,
//                       std::vector<double> logA,
//                       int n, int C, int m, int root,
//                       const std::vector<int> nodes_skip = std::vector<int>()) {
//     // Allocate memory for messages and temporary state values.
//     std::vector<double> log_messages(C * n, 0.0);
//     std::vector<double> state_log_values(C);
...
//         // If either the child or parent is flagged to be skipped,
//         // then do not update the log_messages for this edge.
//         if ( std::find(nodes_skip.begin(), nodes_skip.end(), par) != nodes_skip.end()) {
//             continue;
//         }
...
// // wrapper function: It precomputes dimensions and passes them to score_tree_bp2.
// // E: Edge matrix (each row is a (parent, child) pair, 1-indexed from R)
// // logP_list: List of flattened likelihood matrices (each in row-major order)
// // logA: Flattened transition matrix (row-major order)
// // It computes L (number of loci), C (number of states), n (number of nodes),
// // m (number of edges), and root (parent from the last edge).
// // nodes_skip is passed along to bp.
// // [[Rcpp::export]]
// double score_tree_bp_wrapper2(arma::Mat<int> E,
//                               std::vector< std::vector<double> > logP_list,
//                               std::vector<double> logA,
//                               const std::vector<int> nodes_skip = std::vector<int>(), std::vector<double> log_messages) {
//     // Compute number of loci.
//     int L = logP_list.size();
//     // Infer number of states from the transition matrix.
//     int C = std::sqrt(logA.size());
...
//     // Loop over loci; each element in logP_list is a flattened likelihood matrix for one locus.
//     for (int l = 0; l < L; l++) {
//         logZ += score_tree_bp2(E_vec, logP_list[l], logA, n, C, m, root, nodes_skip, log_messages);
//     }
//     return logZ;
// }
