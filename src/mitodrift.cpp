...
//' logSumExp2 function for a vector
//'
//' @param x NumericVector
//' @return double logSumExp of x
//' @export
...
double logSumExp2(const std::vector<double> x) {
    ...
    if (maxv == -std::numeric_limits<double>::infinity()) {
        return maxv; // If all values are -inf, return -inf
    }

    // Compute stable sum of exponentials
    LDOUBLE cumsum = 0.0;
    for (size_t i = 0; i < n; i++) {
        double xi = x[i];
        if (i != maxi && xi > -std::numeric_limits<double>::infinity()) {
            cumsum += EXPL(xi - maxv);
        }
    }
...
//' @export
// [[Rcpp::export]]
double score_tree_bp(const arma::Mat<int> E, const arma::mat logP, const std::vector<double> logA) {
...
//' @export
// [[Rcpp::export]]
double score_tree_bp_wrapper(arma::Mat<int> E, const arma::cube logP, const std::vector<double> logA) {
...
// E is edge matrix of the tree; rows must be in postorder
// logP is the likelihood matrix (character state x node)
// logA is a flattened transition matrix
// output: total log likelihood for the tree graph via the sum product algorithm
// [[Rcpp::export]]
double score_tree_bp2(const std::vector<int> E, const arma::mat logP, const std::vector<double> logA) {

    int n = logP.n_cols; // Number of nodes
    int m = E.size() / 2; // Number of edges (divided by 2 since each edge has two values)
    int C = logP.n_rows; // Number of character states
    ...
    // Step 3: Process nodes in postorder as given by E
    for (size_t i = 0; i < m; i++) {
        int node = E[2 * i + 1]; // Accessing child node (second element of the edge)
        int par = E[2 * i];     // Accessing parent node (first element of the edge)
    ...
    // Step 4: Compute log-partition function at root
    int root = E[2 * (m - 1)]; // Accessing parent of the last edge (root node)
    for (int c = 0; c < C; c++) {
        state_log_values[c] = logP(c, root) + log_messages[c * n + root];
    }
...
// Wrapper function that converts E into a std::vector<int> (flattened 1D vector)
// [[Rcpp::export]]
double score_tree_bp_wrapper2(arma::Mat<int> E, const arma::cube logP, const std::vector<double> logA) {
    
    int L = logP.n_slices; // Number of loci
    double logZ = 0;
    E = E - 1;

    // Convert E into std::vector<int> (flattened 1D vector)
    ...
    // Loop over loci
    for (int l = 0; l < L; l++) {
        logZ += score_tree_bp2(E_vec, logP.slice(l), logA);
    }

    return logZ;
}
// bp3: Belief-propagation function.
...
// [[Rcpp::export]]
double score_tree_bp3(std::vector<int> E,
...
// wrapper3: Wrapper function that precomputes dimensions and passes them to score_tree_bp3.
...
// [[Rcpp::export]]
double score_tree_bp_wrapper3(arma::Mat<int> E,
  ...
  for (int l = 0; l < L; l++) {
    logZ += score_tree_bp3(E_vec, logP_list[l], logA, n, C, m, root);
  }
  return logZ;
}
...
struct score_neighbours_max: public Worker {
    ...
    score_neighbours_max(const arma::Mat<int> E, const arma::cube logP, const std::vector<double> logA, NumericVector scores): 
...
// [[Rcpp::export]]
NumericVector nni_cpp_parallel(arma::Mat<int> E, const arma::cube logP, const std::vector<double> logA) {
